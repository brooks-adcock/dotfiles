...
Commands and settings are tracked at `~/dotfiles` → `github.com:brooks-adcock/dotfiles`. `~/.claude/commands/` and `~/.claude/settings.json` are symlinked there. Push changes from `~/dotfiles` to sync across machines.
